(function(){
  ...
  function showUpdate(reg){
    ...
    style.textContent='#pwaUpdateNotice{position:fixed;top:max(56px,calc(env(safe-area-inset-top) + 46px));right:12px;z-index:9999;display:flex;align-items:center;gap:10px;max-width:min(92vw,420px);padding:10px 11px 10px 13px;background:rgba(17,24,39,.96);color:#fff;border-radius:13px;box-shadow:0 12px 36px rgba(0,0,0,.24);font:600 12px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;backdrop-filter:blur(10px)}#pwaUpdateNotice button{border:0;border-radius:9px;padding:7px 10px;background:#fff;color:#172033;font-weight:850;cursor:pointer;white-space:nowrap}#pwaUpdateNotice button:disabled{opacity:.65;cursor:default}@media(max-width:540px){#pwaUpdateNotice{left:10px;right:10px;top:max(52px,calc(env(safe-area-inset-top) + 44px));max-width:none;justify-content:space-between}}';
